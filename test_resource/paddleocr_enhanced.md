...
![PaddleOCR Architecture](https://raw.githubusercontent.com/cuicheng01/PaddleX_doc_images/main/images/paddleocr/README/Arch_cn.jpg)

<details>
<summary>🤖 <b>AI 图像/表格解析</b></summary>

<image_analysis>
- **核心总结**: 该图像通过分层架构图，系统性地展示了PaddleOCR框架的核心能力与生态体系。它从上至下依次说明了PaddleOCR的典型应用场景（如票据审查、金融分析）、提供的模型库（通用OCR、文档解析、智能理解等系列模型）、开发工具链（训练、推理、部署）、底层技术栈（PaddlePaddle框架）及硬件支持（英伟达、英特尔等）。此图旨在为读者提供一个全局视角，直观理解PaddleOCR作为一个完整解决方案的模块化构成与技术纵深，从而支撑本章节关于‘通用文本识别（场景OCR）’核心特性的论述。
- **关键实体**: Arch_cn
- **简短摘要**: 
</image_analysis>
</details>
...
### 📣 最新动态|🔥 PaddleOCR v3.5.0 发布：推理后端更灵活，文档输出更丰富
...
### 📣 最新动态|🔥 PaddleOCR v3.5.0 发布：推理后端更灵活，文档输出更丰富|Part 1
- **发布PaddleOCR-VL**：
- **模型介绍**：
...
### 📣 最新动态|🔥 PaddleOCR v3.5.0 发布：推理后端更灵活，文档输出更丰富|Part 2
- **PaddleOCR-VL** 是一款先进、高效的文档解析模型，专为文档中的元素识别设计。其核心组件为 PaddleOCR-VL-0.9B，这是一种紧凑而强大的视觉语言模型（VLM），它由 NaViT 风格的动态分辨率视觉编码器与 ERNIE-4.5-0.3B 语言模型组成，能够实现精准的元素识别。**该模型支持 109 种语言，并在识别复杂元素（如文本、表格、公式和图表）方面表现出色，同时保持极低的资源消耗。通过在广泛使用的公开基准与内部基准上的全面评测，PaddleOCR-VL 在页级级文档解析与元素级识别均达到 SOTA 表现**。它显著优于现有的基于Pipeline方案和文档解析多模态方案以及先进的通用多模态大模型，并具备更快的推理速度。这些优势使其非常适合在真实场景中落地部署。模型已发布至[HuggingFace](https://huggingface.co/PaddlePaddle/PaddleOCR-VL)，欢迎大家下载使用！更多介绍内容请点击[PaddleOCR-VL](https://www.paddleocr.ai/latest/version3.x/algorithm/PaddleOCR-VL/PaddleOCR-VL.html)。

---

### 📣 最新动态|🔥 PaddleOCR v3.5.0 发布：推理后端更灵活，文档输出更丰富|Part 3
- **PaddleOCR-VL** 是一款先进、高效的文档解析模型，专为文档中的元素识别设计。其核心组件为 PaddleOCR-VL-0.9B，这是一种紧凑而强大的视觉语言模型（VLM），它由 NaViT 风格的动态分辨率视觉编码器与 ERNIE-4.5-0.3B 语言模型组成，能够实现精准的元素识别。**该模型支持 109 种语言，并在识别复杂元素（如文本、表格、公式和图表）方面表现出色，同时保持极低的资源消耗。通过在广泛使用的公开基准与内部基准上的全面评测，PaddleOCR-VL 在页级级文档解析与元素级识别均达到 SOTA 表现**。它显著优于现有的基于Pipeline方案和文档解析多模态方案以及先进的通用多模态大模型，并具备更快的推理速度。这些优势使其非常适合在真实场景中落地部署。模型已发布至[HuggingFace](https://huggingface.co/PaddlePaddle/PaddleOCR-VL)，欢迎大家下载使用！更多介绍内容请点击[PaddleOCR-VL](https://www.paddleocr.ai/latest/version3.x/algorithm/PaddleOCR-VL/PaddleOCR-VL.html)。
...
<image_analysis>
- **核心总结**: 该表格旨在引导读者通过扫描两个二维码，分别关注 PaddlePaddle 官方微信公众号和加入其技术讨论群，从而建立与社区的连接。在‘社区’章节的语境下，此表作为社区参与入口，强化用户与开源项目生态的互动，推动技术交流与资源获取，是构建活跃开发者社区的重要工具。
- **关键实体**: table_entity
- **简短摘要**: 该表提供两个二维码入口，分别用于关注 PaddlePaddle 官方微信公众号和加入技术讨论群，旨在促进用户参与社区交流与技术互动。
</image_analysis>
</details>
...
<image_analysis>
- **核心总结**: 该表格列举了多个基于PaddleOCR技术生态的优秀开源项目，每个项目均附带GitHub链接和星标数，旨在展示PaddleOCR在实际应用中的扩展性与影响力。这些项目涵盖AI工作流引擎、实时流处理、文档转换、批量OCR、多LLM客户端、屏幕解析、问答系统、PDF提取、实时翻译等多个垂直领域，体现了PaddleOCR作为OCR基础引擎在构建智能化文档处理与人机交互系统中的核心地位。表格不仅提供项目概览，还通过链接引导读者深入探索，强化了其作为技术推荐与社区资源聚合的功能。
- **关键实体**: 基于PaddleOCR的优秀项目列表
- **简短摘要**: 该表展示了多个基于PaddleOCR技术构建的实用开源项目，涵盖文档处理、AI工作流、实时翻译等多个场景，旨在推广PaddleOCR生态系统的应用价值与社区活跃度。
</image_analysis>
</details>
