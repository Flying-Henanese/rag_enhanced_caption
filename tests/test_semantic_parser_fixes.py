...
'''
    ...
    chunks = semantic.chunk_markdown(markdown_content, embed_fn=lambda x: [0.0]*len(x))
    
    # 筛选包含列表内容的 Chunk
    list_chunks = [c for c in chunks if "发布PaddleOCR-VL" in c.content]
    assert len(list_chunks) == 1, "嵌套列表应该被完整提取为一个 Chunk，而不是被错误切分成多个或产生冗余分段。"
    
    chunk = list_chunks[0]
    assert chunk.element_type in ["bullet_list_open", "ordered_list_open"], "元素类型应该被识别为列表。"
    ...
    # 验证内容没有内部翻倍重复
    assert chunk.content.count("发布PaddleOCR-VL") == 1, "列表内容发生内部翻倍重复（这是修护前的典型 Bug）！"
...
'''
    ...
    has_virtual_heading = any("PaddleOCR 3.3.0 发布" in header for header in list_chunk.header_path)
    assert has_virtual_heading, "Markdown 列表的 header_path 未能继承 <summary> 定义的虚拟标题上下文！"
