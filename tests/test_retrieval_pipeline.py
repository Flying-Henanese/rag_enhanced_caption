...
logging.getLogger("llama_index.core.retrievers.auto_merging_retriever").setLevel(
    logging.DEBUG
)
...
def test_multi_vector_retrieval_pipeline(mock_markdown_content):
    ...
    index, storage_context, ui_results = asyncio.run(
        build_advanced_rag_index(
    ...
    print(
        f"  - 文档库(Docstore)包含 {len(storage_context.docstore.docs)} 个全量节点 (Chunks, Elements, Path Nodes)。"
    )
    ...
    for node_id, node in storage_context.docstore.docs.items():
        if hasattr(node, "index_id") and node.index_id:  # 这是一个 IndexNode（指针）
            print(
                f"   👉 [指针节点] ID: {node.id_} | 摘要: {node.text[:20]}... | 指向回表ID: {node.index_id}"
            )

    # 2. 模拟一次查询检索
    print("\n\n" + "=" * 60)
    print(
        "🚀 [Step 2] 开始四级火箭检索测试 (Vector -> Recursive -> Rerank -> AutoMerging)"
    )
    print("=" * 60)
